...
def download_one(model, verbose):
...
class OllamaModels:
    ...
    @clock
    def download_many(self, models, concur_req):
        name_ = f"{self.name_}: {inspect.currentframe().f_code.co_name}"
        verbose = True
        counter: Counter[DownloadStatus] = Counter()
        with ProcessPoolExecutor(max_workers=concur_req) as executor:
            to_do_map = {}
            for model in sorted(models):
                future = executor.submit(download_one, model, verbose)
    ...
    @clock
    def save_models_file(self, models):
        name_ = f"{self.name_}: {inspect.currentframe().f_code.co_name}"
...
#       print("{name_} models_data", pformat(models_data))
        ...
        print("{name_} models_all_downloaded_info len", len(models_all_downloaded_info))
        print("{name_} models_info_info len", len(models_big_info))
        print("{name_} models_all_info len", len(models_all_info))
        print("{name_} models_codes_info len", len(models_codes_info))
        print("{name_} models_sql_info len", len(models_sql_info))
        print("{name_} models_math_info len", len(models_math_info))
...
#       printit(f"{name_} models", models)
        concur_req = min(default_concur_req, max_concur_req, len(models))
        printit("{name_} concur_req", concur_req)
        models = [m for m in models if m not in ["wizardlm"]]
        # Download all models files from Ollama
        self.download_many(models, concur_req)
        # Save models file info to disk
        self.save_models_file(models)

    @clock
    def save_file(self, fname, fdata):
        name_ = f"{self.name_}: {inspect.currentframe().f_code.co_name}"
        #fname = os.path.join(self.options["models_dir"], fname)
        print(f"{name_} YYY0 fname {fname}")
        dirpath = os.path.dirname(os.path.abspath(__file__))
        print(f"{name_} YYY1 dirpath {dirpath}")
        fname = os.path.join(dirpath, self.options["models_dir"], fname)
        print(f"{name_} YYY2 fname {fname}")
    ...
    def load_file(self, fname):
        name_ = f"{self.name_}: {inspect.currentframe().f_code.co_name}"
        #fname = os.path.join(self.options["models_dir"], fname)
        print(f"{name_} XXX0 fname {fname}")
        dirpath = os.path.dirname(os.path.abspath(__file__))
        print(f"{name_} XXX1 dirpath {dirpath}")
        fname = os.path.join(dirpath, self.options["models_dir"], fname)
        print(f"{name_} XXX2 fname {fname}")
